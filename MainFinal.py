...
while True:
    ...
    faces = detector(
        gray)  # we run the default face detector we imported on gray(our frame) and assign the value to a new vairable faces
    ...
    for face in faces:
        ...
        cv2.rectangle(face_frame, (x1, y1), (x2, y2), (0, 255, 0),
                      2)  # this is the actual line where we draw the rectangle(green in color. i.e (0,255,0)) on the detected face

        landmarks = predictor(gray,
                              face)  # this uses the face detection file we imported to predict the landmarks of the features of the users face
        ...
        if (left_blink == 0 or right_blink == 0):
            ...
            drowsy = 0
            active = 0
            sound_file = vlc.MediaPlayer("/Users/naol/PycharmProjects/DrawsyDetector/sleeping.aiff")
            if (sleepy > 6):
            ...
            active = 0
            drowsy += 1
            sound_file = vlc.MediaPlayer('/Users/naol/PycharmProjects/DrawsyDetector/drowsy.aiff')
            if (drowsy > 6):
                status = "DROWSY....ALERT !"
